...
output_name = "twelve_shot_{}_zh_prompt.txt".format(test_lang)
...
def XNLI_eval(premise, hypothesis, verbose=False):
    ...
    example_text = "\n".join(["{} ，是吗？{} {}".format(ex_premise, "是的" if ex_label == 'entailment' else "不是的" if ex_label == 'contradiction' else "此外", ex_hypothesis) for ex_premise, ex_hypothesis, ex_label in examples])

    prompt1 = example_text + "\n" + premise + " ，是吗？是的，" + hypothesis
    prompt2 = example_text + "\n" + premise + " ，是吗？不是的， " + hypothesis
    prompt3 = example_text + "\n" + premise + " ，是吗？此外，" + hypothesis
